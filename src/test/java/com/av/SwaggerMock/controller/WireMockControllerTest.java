...
public class WireMockControllerTest {
    ...
    @MockitoBean
    private OpenApiToWireMockService openApiToWireMockService;

    @Test
    void addStubMapping_shouldReturnOkOnValidMapping() throws Exception {
        StubMapping mapping = StubMapping.buildFrom("{ \"request\": { \"method\": \"GET\", \"url\": \"/test\" }, \"response\": { \"status\": 200 } }");
        String json = Json.write(mapping);

        when(wireMockManager.getServer()).thenReturn(mock(WireMockServer.class));

        mockMvc.perform(post("/wiremock/upload-mapping")
    ...
    void uploadOpenApiSpec_shouldReturnOkAndAddMappings() throws Exception {
        ...
        StubMapping mapping = StubMapping.buildFrom("{ \"request\": { \"method\": \"GET\", \"url\": \"/dummy\" }, \"response\": { \"status\": 200 } }");

        when(openApiToWireMockService.generateStubMappings(dummySpec)).thenReturn(Collections.singletonList(mapping));

        mockMvc.perform(post("/wiremock/upload-spec")
                .contentType(MediaType.APPLICATION_JSON)
                .content(dummySpec))
            .andExpect(status().isOk())
